...
import { insertMany, getStats, exportQuery } from "./db.js";
...
// Helpers to normalize WU responses
function getListFromJson(j) {
  if (Array.isArray(j)) return j;
  if (j && Array.isArray(j.observations)) return j.observations;
  return [];
}
...
app.get("/api/db/export.csv", (req, res) => {
  try {
    ...
    const cols = ["station","epoch","obsTimeUtc","tempC","dewpointC","humidity","pressureHpa","windKph","windGustKph","windDir","precipRateMm","precipTotalMm","solarWm2","uv"];
    const esc = (v) => {
